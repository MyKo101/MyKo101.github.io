...
However, thanks to GitHub user [`s0`](https://github.com/s0) and their GitHub Action, it's possible to keep your work inside the `doc` folder on your `default` branch (whatever it's name may be) and have that folder automatically pushed to the `gh-branch`.
...
We then give a bit of information about the job, first it's name `Deploy to GitHub Pages` (more like a title), followed by what operating system we want GitHub Actions to use to implement it. Finally, we have the `steps:`, which is where we put the list of tasks that need to be run (in order).
