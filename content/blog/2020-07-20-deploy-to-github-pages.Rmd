...
After you've done this locally, you'll have to `git push` your repo up to GitHub again, and then you'll have to change GitHub's default branch to your new one in the Settings as above.
...
The `on:` part contains information on what events will trigger the event. Here, we're telling GitHub that when we push our repo to the `main` branch, if something has changed in the `docs` folder (or path). This means that any pushes that happen to other branches will be ignored, and any pushes that don't change our `docs` folder, will also be ignored. This is useful because we're building our `gh-pages` branch based solely on what's in `docs`. If something changes elsewhere in the repo, it doesn't matter (unless of course, those changes cause effects in the `docs` folder, which will still cause the trigger). It also doesn't matter what happens on other branches (such as a `devel` branch) because we're not wanting to build our GitHub pages from them.
