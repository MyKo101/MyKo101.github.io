...
Factors also take up slightly less space in your system's memory. In R, this is approximately half the space of a factor, however the way R stores this kind of data is surprisingly efficient. This also means that working with factor variables is often much faster.
...
* `%<>%` grabs the tibble on it's left hand side and passes it to the function on the right. At this point, it works exactly like the regular `%>%` operator.
    * `mutate()` means we are creating or updating a variable inside the tibble
        * `across()` allows us perform a function _across_ many variables within the tibble. These functions will be applied and then overwrite the variable within the tibble
            * `where()` allows us to specify _where_ we want `across()` to perform the function, this is passed as the first argument to `across()`
            ...
            * `where()` therefore returns a vector of `TRUE` and `FALSE` to indicate which variables in the tibble we want `across()` to act on
            * `as_factor()` converts things (e.g. characters) into factors.
        * `across()` has therefore been passed a logical vector telling it where to apply a function and a function that it needs to apply. So it does just that and outputs another tibble.
    * `mutate()` has then been passed a tibble for it's first argument (`df` via the `%<>%` pipe) and another tibble as the output of `across()`. It stitches these together, if there are any names in common, it overwrites those in `df` with those from `across()`
* `%<>%` then recieves this new tibble from `mutate()` and stores it back into the `df` tibble that we originally passed to it. It overwrites this original with the new, updated one. That's why this is called the _assignment pipe_ or _updating pipe_.